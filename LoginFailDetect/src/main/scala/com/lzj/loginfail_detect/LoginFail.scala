...
//输出的异常报警信息样例类
case class Waring(userId: Long, firstFailTime: Long, lastFailTime: Long, waringMsg: String)
...
class LoginWaring(maxFailTimes: Int) extends KeyedProcessFunction[Long, LoginEvent, Waring] {
  //定义状态,保存2秒内的所有失败事件
  ...
  override def processElement(value: LoginEvent, ctx: KeyedProcessFunction[Long, LoginEvent, Waring]#Context, out: Collector[Waring]): Unit = {
    ...
    if (value.eventType == "fail") {
      ...
      if (iter.hasNext) {
        ...
        if (value.eventTime < firstFail.eventTime + 2) {
          //如果两次间隔小于2秒,输出报警
          out.collect(Waring(value.userId, firstFail.eventTime, value.eventTime, "login fail in 2 seconds"))
        }
  ...
  /*override def onTimer(timestamp: Long, ctx: KeyedProcessFunction[Long, LoginEvent, Waring]#OnTimerContext, out: Collector[Waring]): Unit = {
    ...
    if (allLoginFails.length >= maxFailTimes) {
      out.collect(Waring(allLoginFails.head.userId, allLoginFails.head.eventTime, allLoginFails.last.eventTime, "login fail in 2 seconds for " + allLoginFails.length + ".times"))
    }
